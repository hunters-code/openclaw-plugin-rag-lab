...
import type { CreateOrbitSdkConfig } from "@orbit-0g/sdk";
...
async function getPluginId(): Promise<Hex> {
...
async function billUsage(toolName: string) {
  const pluginId = await getPluginId();
...
export default definePluginEntry({
  ...
  description:
    "Daftar tugas ringan in-memory per proses gateway: tambah, lihat, selesai",
  register(api) {
    api.registerTool({
      ...
      async execute(_id, params) {
        try {
          ...
          const p = params as Static<typeof simpleTaskAddParams>;
    ...
    api.registerTool({
      ...
      async execute(_id, params) {
        try {
          ...
          const p = params as Static<typeof simpleTaskListParams>;
    ...
    api.registerTool({
      ...
      async execute(_id, params) {
        try {
          ...
          const p = params as Static<typeof simpleTaskCompleteParams>;
    ...
    api.registerTool(
      {
        ...
        async execute(_id, params) {
          try {
            ...
            const p = params as Static<typeof simpleTaskClearParams>;
            if (p.confirm !== "yes") {
